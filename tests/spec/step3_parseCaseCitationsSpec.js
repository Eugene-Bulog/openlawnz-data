...
const fs = require("fs");
const regNeutralCite = require("../../controller/step3_parseCaseCitations")
	.regDoubleCites;
...
describe("Regex", function() {
	it("Testing section regex", function() {
		const foundSections = citationData.matchAll(sectionsSearch);
	...
	it("Testing explicit definition #1 - RMA in case1", function() {
		const citationDataLegislation = "Resource Management Act 1991 ";
		var leg = citationDataLegislation.concat(explicitDefinitionRegex);
		const explicitDefinition = citationData.match(leg);
	...
	it("Testing tribunal citation regex", function() {
		var tribunalCitations = citationData.match(regNeutralCite);
		// remove errant line breaks
		tribunalCitations = tribunalCitations.map(function(x) {
			return x.replace(/\n/g, " ");
		});
		var tribunalArray = [];
		tribunalArray.push(
